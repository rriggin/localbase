...
class ResidentialPropertyEnrichmentAgent:
    ...
    def get_property_data(self, address: str, city: str, state: str, zip_code: str = None) -> Optional[Dict]:
        ...
        """
        # Property/Detail endpoint - corrected parameter format
        url = f"{self.base_url}/property/detail"
        
        # Use the correct parameter names based on ATTOM API docs
        params = {
            "address1": address,
            "address2": f"{city} {state}",
        }
        
        # Note: postalcode parameter causes "Invalid Parameter Combination" error
        # So we don't include zip_code in the API call
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                if data and data.get('property') and len(data['property']) > 0:
    ...
    def get_property_avm(self, address: str, city: str, state: str, zip_code: str = None) -> Optional[Dict]:
        ...
        """
        # AVM endpoint - corrected parameter format
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                if data and data.get('avm') and len(data['avm']) > 0:
                    return data['avm'][0]
                else:
    ...
    def process_address_list(self, csv_file_path: str, output_file_path: str = None, max_records: int = None) -> pd.DataFrame:
        ...
        for index, row in df.iterrows():
            ...
            if property_data:
                # Get AVM estimate - temporarily disabled due to 404 errors
...
def main():
    ...
    test_address = "2325 W Country Club Dr"
    test_city = "Sedalia"
    test_state = "MO"
    test_zip = "65301"
    
    logger.info("Testing single address lookup...")
    property_data = agent.get_property_data(test_address, test_city, test_state, test_zip)
    
    if property_data:
        print("✅ Property data found!")
        enriched = agent.enrich_property_data(property_data)
        print(f"Year built: {enriched.get('year_built')}")
        print(f"Property type: {enriched.get('property_type')}")
    else:
